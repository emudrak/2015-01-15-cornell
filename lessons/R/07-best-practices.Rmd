...
function or get help. You should periodically clear your environment and re-create it from the code in your script to make sure that the objects you are working can be reproduced if you sent the script to someone else. For that reason, when R asks you to save your Workpace image (.Rdata) don't save it. 
...
### I want to know what kinds of packages are available and what analyses are appropriate for data particular to my field

[R task views](http://cran.r-project.org/web/views/) are reviews of R packages conducted by domain experts. They provide a summary of different package that can do what functions related to specific sub-areas of many fields. 

### I found a package that I want to use, but I feel I need a tutorial for it. 

Head to the [CRAN page](http://cran.r-project.org/web/packages/available_packages_by_name.html) for it. Look under the downloads section.  

All packages will have a Reference Manual.  This is a pdf of all the functions in that package with all of the documentation for each function (the same as if you had typed ?functionName in the console). 

Sometimes listed after the Reference Manual is a **"Vignette."** If there is, you're in luck!  This is a tutorial that the package author has written.  

Sometimes there are also web pages associated with packages. Those are also good places to look for help. 
...
) if your question hasn't been answered before and is well
  crafted, chances are you will get an answer in less than 5 min. They also incentivize posting and answering with reputation points.  
